...
export class IndustryDao {
   ...
   public async fetchCountNotParsed() : Promise<number> {
     return await IndustryModel.find({$and: [{duplicatedWith: null}, {$or: [{rawDataParsed: false}, {rawDataParsed: undefined}]} ]}).count();
   } 
   ...
   public async fetchByCountryCodeAndySlugAndPopulate(countryCode: string, slug: string): Promise<Industry | null> {
     const items = await IndustryModel.find({$and: [{duplicatedWith: null},{$or: [{slug: slug}, {name: slug}]}]}).populate(
          {
               path: 'category',
               populate: 'country'
          }
